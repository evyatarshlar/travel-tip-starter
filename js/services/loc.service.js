    ...
    return storageService.query(DB_KEY)
        .then(locs => {
            if (gFilterBy.txt) {
                ...
                const locsName = locs.filter(loc => regex.test(loc.name))
                const regex1 = new RegExp(gFilterBy.txt, 'i')
                const locsAddress = locs.filter(loc => regex1.test(loc.geo.address))
                locs = locsName.concat(locsAddress)
            }
    ...
    return storageService.query(DB_KEY)
        .then(locs => {
            ...
            const LocCountByLastUpdatedMap = locs.reduce((map, loc) => {
                const secondsPast = Math.round((now - loc.updatedAt) / 1000)
                if (secondsPast < 60*60*24) map.today++
                else if (secondsPast > 60*60*24) map.past++
                else map.never++
                return map
